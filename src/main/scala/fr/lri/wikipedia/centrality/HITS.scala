...
private class HITS extends CentralityMeasure {
  ...
  def calcAuthority(spark: SparkSession,
                    graphDF: org.apache.spark.sql.DataFrame): (org.apache.spark.sql.DataFrame) = {
  ...
  def hits(myGraph:Graph[WikiPage,Link]):(Graph[WikiPage,Link])={
